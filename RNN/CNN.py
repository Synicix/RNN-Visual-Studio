import tensorflow as tf

# ML HW 2
# Daniel Sitonic

def main():

  sess = tf.InteractiveSession()

  # Import MNIST data set
  from tensorflow.examples.tutorials.mnist import input_data
  mnist = input_data.read_data_sets("MNIST_data/", one_hot=True)

  # Variables
  with tf.name_scope("Inputs"):
      x = tf.placeholder(tf.float32, [None, 784], name="Input-Image")
      y_ = tf.placeholder(tf.float32, [None, 10], name="Image-Labels")

  # Building CNN

  # First Convuolution Layer
  with tf.name_scope("CNN-Layer-1"):
      w_conv1 = weight_variable([5, 5, 1, 32])
      b_conv1 = bias_variable([32])

      x_image = tf.reshape(x, [-1, 28, 28, 1])

      h_conv1 = tf.nn.relu(conv2d(x_image, w_conv1) + b_conv1)
      h_pool1 = max_pool_2x2(h_conv1)

      tf.summary.histogram("weights", w_conv1)
      tf.summary.histogram("biases", b_conv1)
      tf.summary.histogram("activations", h_conv1)

  # Second Convolution Layer (64 features for each 5x5 patch)
  with tf.name_scope("CNN-Layer-2"):
      w_conv2 = weight_variable([5, 5, 32, 64])
      b_conv2 = bias_variable([64])

      h_conv2 = tf.nn.relu(conv2d(h_pool1, w_conv2) + b_conv2)
      h_pool2 = max_pool_2x2(h_conv2)

      tf.summary.histogram("weights", w_conv2)
      tf.summary.histogram("biases", b_conv2)
      tf.summary.histogram("activations", h_conv2)

  # Fully Connected Layers
  with tf.name_scope("FCL-1"):
      w_fc1 = weight_variable([7 * 7 * 64, 1024])
      b_fc1 = bias_variable([1024])

      h_pool2_flat = tf.reshape(h_pool2, [-1, 7 * 7 * 64])
      h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, w_fc1) + b_fc1)
      tf.summary.histogram("weights", w_fc1)
      tf.summary.histogram("biases", b_fc1)
      tf.summary.histogram("activations", h_fc1)

  # Dropout (To reduce overfitting)
  with tf.name_scope("Dropout"):
      keep_prob = tf.placeholder(tf.float32, name="keep_prop")
      h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob, name="h_fcl_drop")
      tf.summary.histogram("keep_prob", keep_prob)
      tf.summary.histogram("h_fc1_drop", h_fc1_drop)

  # Readout Layer
  with tf.name_scope("FCL-2-Readout-Layer"):
      w_fc2 = weight_variable([1024, 10])
      b_fc2 = bias_variable([10])

      y_conv = tf.matmul(h_fc1_drop, w_fc2) + b_fc2

      tf.summary.histogram("weights", w_fc2)
      tf.summary.histogram("bias", b_fc2)

  # Training
  with tf.name_scope("x-entropy"):
      cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=y_conv), name="cross_entropy")
      train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)
      correct_prediction = tf.equal(tf.argmax(y_conv, 1), tf.argmax(y_, 1))
      accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32), name="accuracy")

      tf.summary.scalar("cross_entropy", cross_entropy)
      tf.summary.scalar("accuracy", accuracy)

  summary = tf.summary.merge_all()
  writer = tf.summary.FileWriter("/TensorBoard/CNN", sess.graph)

  # Launching Model
  sess.run(tf.global_variables_initializer())

  for i in range(20000):
      batch = mnist.train.next_batch(100)
      if i % 10 == 0:
          train_accuracy = accuracy.eval(feed_dict={x: batch[0], y_: batch[1], keep_prob: 1.0})
          print("step %d, training accuracy %g" % (i, train_accuracy))
          s = sess.run(summary, feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.5})
          writer.add_summary(s, i)
      sess.run(train_step, feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.5})

  print("test accuracy %g" % accuracy.eval(feed_dict={
      x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0}))

# Initialization Functions
def weight_variable(shape):
  initial = tf.truncated_normal(shape, stddev=0.1)
  return tf.Variable(initial)

def bias_variable(shape):
  initial = tf.constant(0.1, shape=shape)
  return tf.Variable(initial)

# Convolution and Pooling Functions
def conv2d(x, W):
  return tf.nn.conv2d(x, W, strides=[1, 1, 1, 1], padding='SAME')

def max_pool_2x2(x):
  return tf.nn.max_pool(x, ksize=[1, 2, 2, 1],
                        strides=[1, 2, 2, 1], padding='SAME')

if __name__ == "__main__":
    main()